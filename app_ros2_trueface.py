    ...
    class PTZControlNode(Node):
        ...
        def __init__(self):
            ...
            self.tracking_status_sub = self.create_subscription(
                String,
                '/ptz_manager/tracking_status',
                self.tracking_status_callback,
                subscriber_qos
            )
            
            # === 新增：发布追踪模式切换命令 ===
            self.tracking_mode_pub = self.create_publisher(
                String,
                '/ptz_manager/tracking_mode',
                publisher_qos
            )
        ...
        def publish_tracking_mode_command(self, mode):
            """发布追踪模式命令"""
            try:
                if not hasattr(self, 'tracking_mode_pub'):
                    return False, "追踪模式发布者未初始化"
                
                # 验证模式参数
                if mode not in ['default', 'specified']:
                    return False, f"无效的追踪模式: {mode}，仅支持 'default' 或 'specified'"
                ...
                self.get_logger().info(f'发布追踪模式命令: {mode} ({mode_names[mode]})')
                
                return True, f'成功切换到{mode_names[mode]}'
                
            except Exception as e:
                self.get_logger().error(f'发布追踪模式命令时出错: {e}')
                return False, f'发布失败: {str(e)}'

        def call_switch_tracking_target_service(self, target_face_id):
            """调用切换追踪目标服务"""
            try:
                # 导入服务类型
                ...
                # 创建服务客户端
                client = self.create_client(SwitchTrackingTarget, '/ptz_manager/switch_tracking_target')
                
                # 等待服务可用
                if not client.wait_for_service(timeout_sec=5.0):
                    return False, "切换追踪目标服务不可用"
                
                # 创建请求
                request = SwitchTrackingTarget.Request()
                request.target_face_id = target_face_id
                
                # 调用服务
                future = client.call_async(request)
                
                # 这里使用简单的等待方式，实际应用中可能需要异步处理
                import time
                timeout = 5.0
                ...
                if future.done():
                    ...
                    if response.success:
                        self.get_logger().info(f'成功切换追踪目标到ID: {target_face_id}')
                        return True, response.message
                    else:
                        self.get_logger().error(f'切换追踪目标失败: {response.message}')
                        return False, response.message
                else:
                    return False, "切换追踪目标服务调用超时"
                    
            except Exception as e:
                self.get_logger().error(f'调用切换追踪目标服务时出错: {e}')
        ...
        def tracking_status_callback(self, msg):
            ...
            try:
                ...
                if hasattr(msg, 'data') and isinstance(msg.data, str):
                    try:
                        status_json = json.loads(msg.data)
                        
                        global tracking_status_data
                        tracking_status_data = {
                            "available_face_ids": status_json.get('available_face_ids', []),
                            "current_tracking_id": status_json.get('current_tracking_id'),
                            "tracking_status": status_json.get('tracking_status', 'idle'),
                            "tracking_mode": status_json.get('tracking_mode', 'default'),
                            "user_selected_id": status_json.get('user_selected_id'),
                            "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                        }
                        
                        # 通过WebSocket发送更新
                        socketio.emit('tracking_status_update', tracking_status_data)
                        self.get_logger().info(
                            f'追踪状态更新: 当前ID={tracking_status_data["current_tracking_id"]}, '
                            f'状态={tracking_status_data["tracking_status"]}, '
                            f'模式={tracking_status_data["tracking_mode"]}'
                        )
                        
                    except json.JSONDecodeError as e:
                        self.get_logger().error(f'解析追踪状态JSON失败: {e}')
                        tracking_status_data = {
                            "available_face_ids": [],
                            "current_tracking_id": None,
                    ...
                    self.get_logger().error('追踪状态消息格式错误')
                    
            except Exception as e:
                self.get_logger().error(f'处理追踪状态消息时出错: {e}')
                tracking_status_data = {
                    "available_face_ids": [],
                    "current_tracking_id": None,
                    "tracking_status": "error",
...
def api_tracking_status():
    ...
    return jsonify({
        'success': True,
        'data': tracking_status_data,
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    })

@app.route('/api/tracking_mode', methods=['POST'])
def api_set_tracking_mode():
    """设置追踪模式的API"""
    try:
        global ros_node
        ...
        if mode not in ['default', 'specified']:
            return jsonify({
                'success': False, 
                'message': f'无效的追踪模式: {mode}，仅支持 "default" 或 "specified"'
            })
        ...
        # 使用ROS节点发布模式命令
        success, message = ros_node.publish_tracking_mode_command(mode)
...
@app.route('/api/switch_tracking_target', methods=['POST'])
def api_switch_tracking_target():
    """切换追踪目标的API"""
    try:
        global ros_node
        
        data = request.get_json()
        target_face_id = data.get('target_face_id')
        
        if target_face_id is None:
            return jsonify({
                'success': False, 
                'message': '缺少target_face_id参数'
            })
        ...
        try:
            ...
            return jsonify({
                'success': False, 
                'message': f'target_face_id必须是整数: {target_face_id}'
            })
        ...
        # 使用ROS节点调用服务
        success, message = ros_node.call_switch_tracking_target_service(target_face_id)
